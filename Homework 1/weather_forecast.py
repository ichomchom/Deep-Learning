...
class WeatherForecast:
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        avg = torch.mean(self.data, dim=1)
        abs_diff = torch.abs(self.data - avg.unsqueeze(1))
        idx = torch.argmax(abs_diff, dim=1)
        max_temp = torch.gather(self.data, dim=1, index=idx.unsqueeze(1))
        return max_temp.squeeze(1)
    ...
    def predict_temperature(self, k: int) -> torch.Tensor:
        past_k_date = self.data[-k:,:]
        return torch.mean(past_k_date)
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        abs_diff = torch.abs(self.data - t)
        sum_diff = torch.sum(abs_diff, dim=1)
        return torch.argmin(sum_diff)
