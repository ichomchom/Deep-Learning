...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        train_average = sum(metrics["train_acc"]) / len(metrics["train_acc"])
        logger.add_scalar("train_accuracy", train_average, global_step)
        ...
        for _ in range(10):
            ...
            # TODO: save additional metrics to be averaged
            metrics["val_acc"].extend(dummy_validation_accuracy)

        # TODO: log average val_accuracy
        val_average = sum(metrics["val_acc"]) / len(metrics["val_acc"])
        logger.add_scalar("val_accuracy", val_average, global_step)
...
if __name__ == "__main__":
    ...
    log_dir = Path(args.exp_dir) / \
        f"logger_{datetime.now().strftime('%m%d_%H%M%S')}"
