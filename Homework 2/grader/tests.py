...
class LogGrader(Grader):
    ...
    def test_train(self):
        ...
        for epoch in range(10):
            ...
            expect = epoch / 10.0 + \
                torch.mean(torch.cat([torch.randn(10) for i in range(20)]))
            assert (
                20 * epoch + 19 in acc or 20 * epoch + 20 in acc
            ), f"No train_accuracy logging found for epoch {epoch}"
            got = acc[20 * epoch + 19] if 20 * epoch + \
                19 in acc else acc[20 * epoch + 20]
            assert abs(
                got - expect) < 1e-2, f"train_accuracy epoch={epoch} expected {expect} got {got}"
    ...
    def test_val(self):
        ...
        for epoch in range(10):
            torch.manual_seed(epoch)
            expect = epoch / 10.0 + \
                torch.mean(torch.cat([torch.randn(10) for i in range(10)]))
            assert 20 * epoch + 19 in acc or 20 * epoch + \
                20 in acc, f"No val_accuracy logging found for epoch {epoch}"
            got = acc[20 * epoch + 19] if 20 * epoch + \
                19 in acc else acc[20 * epoch + 20]
            assert abs(
                got - expect) < 1e-2, f"val_accuracy epoch={epoch} expected {expect} got {got}"
    ...
    def trace(cls, model, input_sample):
        ...
        def graph_tracer(node):
            ...
            if node.kind() in ["prim::Param"]:
                ...
                all_stats = [graph_tracer(i.node()) for i in node.inputs(
                ) if i.node().kind() != "prim::Constant"]
                visited[node] = TracerStats(
...
class LinearGrader(Grader):
    ...
    def accuracy(
        ...
        accuracy_normalized = (accuracy - min_accuracy) / \
