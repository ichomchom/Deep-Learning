...
class TransformerPlanner(nn.Module):
    ...
    def pos_encoding(self, x):
        B, N, _ = x.shape
        pos = torch.linspace(0, 1, N, device=x.device).unsqueeze(
            0).repeat(B, 1).unsqueeze(-1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        attended = self.transformer_decoder(tgt=queries, memory=x)
...
class CNNPlanner(torch.nn.Module):
    def __init__(
        ...
        self.register_buffer("input_mean", torch.as_tensor(
            INPUT_MEAN), persistent=False)
        self.register_buffer("input_std", torch.as_tensor(
            INPUT_STD), persistent=False)
